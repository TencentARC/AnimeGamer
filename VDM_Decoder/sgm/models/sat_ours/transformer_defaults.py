...
def attention_forward_default(self, hidden_states, mask, backend=None, **kw_args):
    ...
    (mixed_query_layer, # [b, 6976, 1920]
        mixed_key_layer,
        mixed_value_layer) = split_tensor_along_last_dim(mixed_raw_layer, self.stride)

    dropout_fn = self.attention_dropout if self.training else None

    query_layer = self._transpose_for_scores(mixed_query_layer) # [2, 6976, 1920] -> [2, 30, 6976, 64])
    ...
    if self.use_attached_ip_cross_attn:
        ip_cond = kw_args['ip_encoder_outputs']
        ip_query_layer = mixed_query_layer.clone()
        ip_key_layer = self.ipca_to_k(ip_cond)
        ip_value_layer = self.ipca_to_v(ip_cond)
        ip_query_layer = self._transpose_for_scores(ip_query_layer)
        ip_key_layer = self._transpose_for_scores(ip_key_layer)
        ip_value_layer = self._transpose_for_scores(ip_value_layer)  

        with sdp_kernel(**BACKEND_MAP[backend]):   
            ip_context_layer = F.scaled_dot_product_attention(ip_query_layer, ip_key_layer, ip_value_layer)

        ip_context_layer = ip_context_layer.permute(0, 2, 1, 3).contiguous()
        ip_context_layer = ip_context_layer.view(*new_context_layer_shape)
        context_layer = context_layer + ip_context_layer 
