...
class SelfAttention(torch.nn.Module):
    def __init__(self, hidden_size, num_attention_heads,
                 ...
                 hooks={}, transformer_pointer=None, params_dtype=torch.float, skip_init=False, device=torch.device('cpu'), use_attached_ip_cross_attn=False):
        ...
        if self.use_attached_ip_cross_attn:
            self.ipca_to_k = nn.Linear(1280, 1920, bias=False)
            self.ipca_to_v = nn.Linear(1280, 1920, bias=False)
...
class BaseTransformerLayer(torch.nn.Module):
    def __init__(
        ...
        self.attention = SelfAttention(
            ...
            skip_init=skip_init,
            device=device,
            use_attached_ip_cross_attn=use_attached_ip_cross_attn,
        )
