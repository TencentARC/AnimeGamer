...
from ...vae_modules.utils import checkpoint
...
class IPCrossAttention(nn.Module):
    def __init__(
        ...
        self.ipca_scale = dim_head**-0.5
        self.ipca_heads = heads

        self.ipca_to_q = nn.Linear(query_dim, inner_dim, bias=False)
        self.ipca_to_k = nn.Linear(query_dim, inner_dim, bias=False)
        self.ipca_to_v = nn.Linear(query_dim, inner_dim, bias=False)

        self.ipca_to_out = nn.Sequential(nn.Linear(inner_dim, query_dim), nn.Dropout(dropout))
        self.ipca_backend = backend
        self.ipca_ca_proj = nn.Sequential(nn.Linear(context_dim, query_dim), nn.LayerNorm(query_dim))
    ...
    def forward(
        ...
        context = self.ipca_ca_proj(context)
        context = default(context, x)
        k = self.ipca_to_k(context)
        v = self.ipca_to_v(context)
        ...
        """
        # new
        with sdp_kernel(**BACKEND_MAP[self.ipca_backend]):
            # print("dispatching into backend", self.backend, "q/k/v shape: ", q.shape, k.shape, v.shape)
            out = F.scaled_dot_product_attention(q, k, v, attn_mask=mask)  # scale is dim_head ** -0.5 per default
        ...
        if additional_tokens is not None:
            # remove additional token
            out = out[:, n_tokens_to_mask:]
        return self.ipca_to_out(out)

class IP_Text_Mix_Attention(nn.Module):
    def __init__(
        self,
        query_dim,
        context_dim=None,
        ...
        self.use_LN = use_LN
        if self.use_LN:
            self.text_ln = nn.LayerNorm(query_dim)
            self.ip_ln = nn.LayerNorm(context_dim)
    def forward(
        self,
        x,
        context=None,
...
def PerceiveFeedForward(dim, mult=4):
    inner_dim = int(dim * mult)
...
class IP_Text_Mix_PerceiverAttention(nn.Module):
    def __init__(
        self,
        dim=1920,
        depth=4,
        ...
        for _ in range(depth):
            self.layers.append(
                nn.ModuleList([
                    ...
                    PerceiveFeedForward(dim=dim, mult=ff_mult),
                ]))

    def forward(self, latents=None, x=None):
        # latents = self.latents.repeat(x.size(0), 1, 1)
        # print('Perceiver attentioning')
        x = self.proj_in(x)
