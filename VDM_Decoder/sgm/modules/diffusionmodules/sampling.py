...
DEFAULT_GUIDER = {"target": "..diffusionmodules.guiders.IdentityGuider"}
...
class VideoDDIMSampler(BaseDiffusionSampler):
    ...
    def denoise(self, x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep=None, idx=None, scale=None, scale_emb=None, flow=None, aroutput=None):
        additional_model_inputs = {}

        if flow:
            additional_model_inputs["flow_idx"] = torch.cat([x.new_ones([x.shape[0]]) * flow] * 2)
        if aroutput is not None:
            additional_model_inputs["aroutput"] = aroutput
...
class VPSDEDPMPP2MSampler(VideoDDIMSampler):
    ...
    def sampler_step(
        ...
        denoised = self.denoise(
            x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep, idx, scale=scale, scale_emb=scale_emb, flow=flow, aroutput=aroutput
        ).to(torch.float32)
        if idx == 1:
    ...
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, scale=None, scale_emb=None, flow=None, aroutput=None):
        x, s_in, alpha_cumprod_sqrt, num_sigmas, cond, uc, timesteps = self.prepare_sampling_loop(
            x, cond, uc, num_steps
        )
