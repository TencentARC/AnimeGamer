...
OPENAIUNETWRAPPER = "VDM_Decoder.sgm.modules.diffusionmodules.wrappers.OpenAIWrapper"
...
class OpenAIWrapper(IdentityWrapper):
    def forward(self, x: torch.Tensor, t: torch.Tensor, c: dict, **kwargs) -> torch.Tensor:
        ...
        # breakpoint() # c ['crossattn', 'ip_cond'] kwargs [idx flow_idx ]
        return self.diffusion_model(
            x,
            timesteps=t,
            context=c.get("crossattn", None),
            ip_context=c.get("ip_cond", None),
            face_id_cond=c.get("face_id_cond", None),
            y=c.get("vector", None),
            **kwargs,
        )
