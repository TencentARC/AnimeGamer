...
class SATVideoDiffusionEngine(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        clip_img_encoder_config = model_config.get("clip_img_encoder_config", None)
        ...
        self._init_embedder(clip_img_encoder_config)
    ...
    def disable_untrainable_params(self):
        ...
        new_train_list = ['flow', 'mix_proj', 'ip_proj', 'mix_layer_norm', 'ip_crossattention', 'ipca_to_k', 'ipca_to_v', 'attention_mix', 'adaLN_modulations', 'patch_embed.text_proj']
        
        # For Decoder Adaptation
        # new_train_list = ['flow', 'ip_crossattention', 'ipca_to_k', 'ipca_to_v', 'attention_mix', 'adaLN_modulations']

        #For Encoder Warm-up
        #new_train_list = ['flow','ip_proj','patch_embed.text_proj','mix_layer_norm']
        
        total_trainable = 0
        for n, p in self.named_parameters():
            
            if any(item in n for item in new_train_list):
                print(f'[NEW MODULE] {n}')
    ...
    def _init_embedder(self, config, freeze=True):
        self.clip_img_encoder = instantiate_from_config(config)
        if freeze:
            self.clip_img_encoder.eval()
            self.clip_img_encoder.train = disabled_train
    ...
    def forward(self, x, batch): #CJH Here is to pass the loss function
        # breakpoint()
        loss = self.loss_fn(self.model, self.denoiser, self.conditioner, self.clip_img_encoder, x, batch)
        loss_mean = loss.mean()
    ...
    def shared_step(self, batch: Dict) -> Any:
        ...
        x = self.encode_first_stage(x, batch) # x [2, 3, 49, 480, 720] batch: fps [8,8] 
        x = x.permute(0, 2, 1, 3, 4).contiguous() 
        if self.noised_image_input:
    ...
    def sample(
        ...
        **kwargs,
        ...
        samples = self.sampler(denoiser, randn, cond, uc=uc, scale=scale, scale_emb=scale_emb, flow=flow, aroutput=aroutput)
    ...
    def log_video(
        ...
        **kwargs,
        ...
        if self.noised_image_input:
            ...
            c['ip_cond'] = self.clip_img_encoder(batch['ip_img'])
            ...
            if 'face_id' in batch.keys():
                if len(batch['face_id']) != 0:
                    for i in range(len(batch['face_id'])):
                        if i == 0 :
                            face_id_cond = self.clip_img_encoder(batch['face_id'][i].to(self.device))
                        else:
                            face_id_cond = torch.cat((face_id_cond, self.clip_img_encoder(batch['face_id'][i].to(self.device))), dim=1)
                    c['face_id_cond'] = face_id_cond
                    uc['face_id_cond'] = torch.zeros_like(face_id_cond)

            samples = self.sample(c, shape=z.shape[1:], uc=uc, batch_size=N, flow = batch['motion'], **sampling_kwargs)  # b t c h w
            samples = samples.permute(0, 2, 1, 3, 4).contiguous()
            if only_log_video_latents:
                latents = 1.0 / self.scale_factor * samples
                log["latents"] = latents
